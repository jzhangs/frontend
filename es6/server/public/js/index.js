...
var _lottery = __webpack_require__(3);

var _lottery2 = _interopRequireDefault(_lottery);

function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }

var lottery = new _lottery2.default();
...
var Lottery = function (_mix) {
  ...
  function Lottery() {
    ...
    _this.playList = new Map();
    _this.number = new Set();
    _this.issueEl = '#curr_issue';
    _this.countdownEl = '#countdown';
    _this.stateEl = '.state_el';
    _this.cartEl = '.codelist';
    _this.omitEl = '';
    _this.curPlay = 'r5';
    _this.initPlayList();
    _this.initNumber();
  ...
  _createClass(Lottery, [{
    ...
    value: function updateState() {
      ...
      this.getState().then(function (res) {
        ...
        _this2.endTime = res.endTime;
        _this2.state = res.state;
        (0, _jquery2.default)(_this2.issueEl).text(res.issue);
        _this2.countdown(res.endTime, function (time) {
          (0, _jquery2.default)(_this2.countdownEl).html(time);
        }, function () {
          setTimeout(function () {
            _this2.updateState();
...
var Base = function () {
  ...
  _createClass(Base, [{
    ...
    value: function setOmit(omit) {
      ...
      (0, _jquery2.default)(this.omitEl).each(function (index, item) {
    ...
    value: function changePlayNav(e) {
      ...
      (0, _jquery2.default)('#zx_sm span').html(this.playList.get(this.curPlay).tip);
    ...
    value: function assistHandle(e) {
      ...
      var $cur = (0, _jquery2.default)(e.currentTarget);
      var index = $cur.index();
      var $btns = (0, _jquery2.default)('.boll-list .btn-boll');
    ...
    value: function addCode() {
      ...
      var count = this.computeCount(active, this.curPlay);

      if (count) {
        this.addCodeItem($active.join(' '), this.curPlay, this.playList.get(this.curPlay).name, count);
      }
    }
  }, {
    key: 'addCodeItem',
    value: function addCodeItem(code, type, typename, count) {
      ...
      (0, _jquery2.default)(this.cartEl).append(tpl);
      this.getTotal();
    }
  }, {
    key: 'getCount',
    value: function getCount() {
      ...
      var count = this.computeCount(active, this.curPlay);
      var range = this.computeBonus(active, this.curPlay);
    ...
    value: function getTotal() {
      ...
      (0, _jquery2.default)('.codelist li').each(function (index, item) {
        count += parseInt((0, _jquery2.default)(item).attr('count'));
      });
    ...
    value: function getRandomCode(e) {
      ...
      var play = this.curPlay.match(/\d+/g)[0];

      if (num === '0') {
        (0, _jquery2.default)(this.cartEl).html('');
      } else {
        for (var i = 0; i < num; i++) {
          this.addCodeItem(this.getRandom(play), this.curPlay, this.playList.get(this.curPlay).name, 1);
        }
      }
    }
  }]);
  ...
  _createClass(Timer, [{
    ...
    value: function countdown(end, update, handle) {
      ...
      if (now > end) {
        ...
        var d = Math.floor(lastTime / msDay);
        var h = Math.floor((lastTime - d * msDay) / msHour);
        var m = Math.floor((lastTime - d * msDay - h * msHour) / msMin);
        var s = Math.floor((lastTime - d * msDay - h * msHour - m * msMin) / msSec);
        var r = [];
...
var Calculate = function () {
  ...
  _createClass(Calculate, [{
     ...
     * @return {number}
     */
    value: function computeCount(active, playName) {
      var count = 0;
      var exist = this.playList.has(playName);
    ...
    key: 'computeBonus',
    value: function computeBonus(active, playName) {
      var _this = this;

      var size = parseInt(playName[1]);
      var arr = new Array(size).fill(0);
      var min = void 0,
          max = void 0;
      if (playName[0] === 'r') {
        var minActive = 5 - (11 - active);
        if (minActive > 0) {
          if (minActive - size >= 0) {
        ...
        var maxActive = Math.min(active, 5);
        if (size > 5) {
          if (active - size > 0) {
            arr = new Array(active - 5).fill(0);
      ...
      return [min, max].map(function (item) {
        return item * _this.playList.get(playName).bonus;
      });
    }
    ...
    value: function combine(arr, size) {
      ...
      (function f(arr, size, result) {
        ...
        if (size > len) {
          ...
          res.push([].concat(_toConsumableArray(result), _toConsumableArray(arr)));
        } else {
          for (var i = 0; i < len; i++) {
            var nextResult = [].concat(_toConsumableArray(result), [arr[i]]);
            if (size === 1) {
              res.push(nextResult);
            } else {
              var nextArr = [].concat(_toConsumableArray(arr));
